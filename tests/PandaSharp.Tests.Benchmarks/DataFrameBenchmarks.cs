...
namespace PandaSharp.Tests.Benchmarks;
...
public class DataFrameBenchmarks
{
    ...
    private PandaSharp.IO.Database.SqlGenerator _sqlGen = null!;
    private PandaSharp.Lazy.LogicalPlan _sqlPlan = null!;

    [GlobalSetup(Target = nameof(SqlGenerate_FullChain))]
    public void SetupSql()
    {
        Setup();
        _sqlGen = new PandaSharp.IO.Database.SqlGenerator(new PandaSharp.IO.Database.PostgresDialect());
        // Build a realistic plan: Filter → Select → Sort → Head
        var scan = new PandaSharp.Lazy.ScanPlan(_df);
        var filter = new PandaSharp.Lazy.FilterPlan(scan,
            Expressions.Expr.Col("Value") > Expressions.Expr.Lit(500));
        var select = new PandaSharp.Lazy.SelectPlan(filter, ["Id", "Value", "Category"]);
        var sort = new PandaSharp.Lazy.SortPlan(select, "Value", false);
        _sqlPlan = new PandaSharp.Lazy.HeadPlan(sort, 100);
    }
    ...
    private PandaSharp.Geo.GeoColumn _geoCol = null!;
    private PandaSharp.Geo.GeoColumn _geoSmall = null!;
    ...
    public void SetupGeo()
    {
        ...
        _geoCol = new PandaSharp.Geo.GeoColumn("loc", lats, lons);
        ...
        _geoSmall = new PandaSharp.Geo.GeoColumn("targets", slats, slons);
    }

    [Benchmark]
    public PandaSharp.Column.Column<double> GeoDistanceTo()
    {
        return _geoCol.DistanceTo(new PandaSharp.Geo.GeoPoint(40.7128, -74.0060));
    }

    [Benchmark]
    public bool[] GeoWithinDistance()
    {
        return _geoCol.WithinDistance(new PandaSharp.Geo.GeoPoint(40.7128, -74.0060), 500);
    }

    // -- R-tree spatial index --

    private PandaSharp.Geo.RTree _rtree = null!;

    [GlobalSetup(Targets = [nameof(RTreeNearest), nameof(RTreeQueryRadius)])]
    public void SetupRTree()
    {
        SetupGeo();
        _rtree = PandaSharp.Geo.RTree.Build(_geoCol);
    }

    [Benchmark]
    public (int, double) RTreeNearest()
    {
        return _rtree.Nearest(new PandaSharp.Geo.GeoPoint(40.7128, -74.0060));
    }

    [Benchmark]
    public List<(int, double)> RTreeQueryRadius()
    {
        return _rtree.QueryRadius(new PandaSharp.Geo.GeoPoint(40.7128, -74.0060), 500);
    }
    ...
    [Benchmark]
    public PandaSharp.Statistics.DataProfile Profile() => _df.Profile();
    ...
    {
        Setup();
        var cols = new List<PandaSharp.Column.IColumn>();
    ...
    {
        return _wideDf.Lazy()
            .Filter(PandaSharp.Expressions.Expr.Col("col25") > PandaSharp.Expressions.Expr.Lit(0.5))
            .Sort("col10")
            .Select("col0", "col1")
            .Head(100)
            .Collect();
