...
namespace PandaSharp.Text.Embeddings;
...
/// Actual inference requires ONNX Runtime, which will be integrated via PandaSharp.ML.Onnx.
/// </summary>
public sealed class TextEmbedder
{
    ...
    /// Install PandaSharp.ML.Onnx and provide a model file.
    /// </exception>
    public Tensor<float> Embed(string[] texts)
    {
        throw new NotSupportedException(
            "TextEmbedder.Embed() requires ONNX Runtime for model inference. " +
            "Install PandaSharp.ML.Onnx and use OnnxModelRunner to load the model at: " + _modelPath);
    }
