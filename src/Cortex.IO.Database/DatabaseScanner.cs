...
/// <summary>
/// Scan a database table into a Cortex DataFrame with expression push-down.
...
#pragma warning restore CS0414
    ...
    /// </summary>
    public Cortex.Lazy.LazyFrame Lazy()
    {
        var plan = new Cortex.Lazy.ExternalScanPlan(
            fullPlan => ExecutePushDown(fullPlan),
            $"db://{_tableName}");
        return new Cortex.Lazy.LazyFrame(plan);
    }
    ...
    /// </summary>
    private DataFrame ExecutePushDown(Cortex.Lazy.LogicalPlan fullPlan)
    {
        var generator = new SqlGenerator(_dialect);
        try
        {
        ...
        catch (NotSupportedException)
        {
            ...
            return Cortex.Lazy.Executor.ExecuteLocal(rewrittenPlan);
        }
    }
    ...
    /// </summary>
    private static Cortex.Lazy.LogicalPlan RewriteWithLocalScan(
        Cortex.Lazy.LogicalPlan plan, DataFrame data)
    {
        return plan switch
        {
            Cortex.Lazy.ExternalScanPlan => new Cortex.Lazy.ScanPlan(data),
            Cortex.Lazy.SelectPlan s => s with { Input = RewriteWithLocalScan(s.Input, data) },
            Cortex.Lazy.FilterPlan f => f with { Input = RewriteWithLocalScan(f.Input, data) },
            Cortex.Lazy.SortPlan s => s with { Input = RewriteWithLocalScan(s.Input, data) },
            Cortex.Lazy.HeadPlan h => h with { Input = RewriteWithLocalScan(h.Input, data) },
            Cortex.Lazy.GroupByAggPlan g => g with { Input = RewriteWithLocalScan(g.Input, data) },
            Cortex.Lazy.WithColumnPlan w => w with { Input = RewriteWithLocalScan(w.Input, data) },
            _ => plan
        };
    }
    ...
    /// </summary>
    public Cortex.Lazy.LazyFrame LazyJoin(string rightTableName, string onColumn,
        Cortex.Joins.JoinType how = Cortex.Joins.JoinType.Inner)
    {
        var leftPlan = new Cortex.Lazy.ExternalScanPlan(
            fullPlan => ExecutePushDown(fullPlan),
            $"db://{_tableName}");
        var rightPlan = new Cortex.Lazy.ExternalScanPlan(
            fullPlan => ExecutePushDown(fullPlan),
            $"db://{rightTableName}");
        var joinPlan = new Cortex.Lazy.JoinPlan(leftPlan, rightPlan, onColumn, how);

        // Wrap in an ExternalScanPlan that can push the whole thing down
        var executorPlan = new Cortex.Lazy.ExternalScanPlan(
            fullPlan => ExecutePushDown(fullPlan),
            $"db://{_tableName}+{rightTableName}");

        // Return a LazyFrame that, when Collect() is called, uses the join plan
        return new Cortex.Lazy.LazyFrame(joinPlan);
    }
    ...
    /// <summary>
    /// Stream results into a Cortex DataFrame using columnar assembly.
    /// </summary>
