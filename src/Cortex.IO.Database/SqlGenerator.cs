...
public class SqlGenerator
{
    ...
    /// <summary>Compile a Cortex expression to a SQL fragment.</summary>
    ...
    private void WalkPlan(LogicalPlan plan, QueryPlan qp)
    {
        switch (plan)
        {
            ...
            case JoinPlan join:
                ...
                {
                    Cortex.Joins.JoinType.Inner => "INNER JOIN",
                    Cortex.Joins.JoinType.Left => "LEFT JOIN",
                    Cortex.Joins.JoinType.Right => "RIGHT JOIN",
                    Cortex.Joins.JoinType.Outer => "FULL OUTER JOIN",
                    Cortex.Joins.JoinType.Cross => "CROSS JOIN",
    ...
    private class QueryPlan
    {
        ...
        public Cortex.GroupBy.AggFunc? AggFunc { get; set; }
        ...
        public string ToSql(SqlDialect dialect)
        {
            ...
            if (GroupByColumns is not null && AggFunc.HasValue)
            {
                ...
                {
                    Cortex.GroupBy.AggFunc.Sum => "SUM",
                    Cortex.GroupBy.AggFunc.Mean => "AVG",
                    Cortex.GroupBy.AggFunc.Min => "MIN",
                    Cortex.GroupBy.AggFunc.Max => "MAX",
                    Cortex.GroupBy.AggFunc.Count => "COUNT",
                    Cortex.GroupBy.AggFunc.Std => "STDDEV",
                    Cortex.GroupBy.AggFunc.Var => "VARIANCE",
                    Cortex.GroupBy.AggFunc.Median => "PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY",
                    Cortex.GroupBy.AggFunc.First => "MIN", // approximate First as MIN
                    Cortex.GroupBy.AggFunc.Last => "MAX",  // approximate Last as MAX
                ...
                if (SelectColumns is not null)
                {
                    foreach (var col in SelectColumns)
                    {
                        ...
                        {
                            if (AggFunc.Value == Cortex.GroupBy.AggFunc.Median)
                ...
                else
                {
                    ...
                    if (AggFunc.Value == Cortex.GroupBy.AggFunc.Count)
